...
public class RestReadJob extends AbstractRestJob {
    private static final Logger LOGGER = LoggerFactory.getLogger(RestReadJob.class.getName());

    public void perform(Entity entity) {
        if (entity.getEndPoint() == null){
            final String msg = "POST api endpoint can't be null";
            RestReadJob.LOGGER.error(msg,new IllegalArgumentException());
            return;
        }
        ...
        final ValidatableResponse response = httpRequest.when().get(entity.getEndPoint()).then().log().all();
